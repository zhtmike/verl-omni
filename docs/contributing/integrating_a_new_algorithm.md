...
This guide explains how to add a new PPO-like RL algorithm (e.g. FlowGRPO,
DanceGRPO) to VeRL-Omni's diffusion trainer. The contracts described here
are orthogonal to model integration: a single PPO-like algorithm can be
extended to any number of model architectures by pairing it with the
...
- **Subclass the rollout.** Rollout adapters are deep enough that
  ...
  class QwenImageDanceGRPOPipelineWithLogProb(QwenImagePipelineWithLogProb):
      def forward(self, req, *, sde_type="dance", sde_window_size=None, **kw):
