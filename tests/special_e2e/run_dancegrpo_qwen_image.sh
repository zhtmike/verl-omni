#!/usr/bin/env bash
# DanceGRPO diffusion e2e smoke test (minimal runtime), vllm_omni rollout.
#
# Exercises: parquet load -> vllm_omni rollout -> visual reward (jpeg_compressibility,
# no reward model) -> dance_grpo -> FSDP LoRA -> sync.
#
# Differs from the FlowGRPO smoke test only in the algorithm dispatch
# (``algorithm.adv_estimator=dance_grpo``,
#  ``actor_rollout_ref.actor.diffusion_loss.loss_mode=dance_grpo``,
#  ``actor_rollout_ref.rollout.algo.sde_type=dance``) and in the SDE-window
# settings (DanceGRPO has no window — every step injects noise).
#
...
python3 -m verl_omni.trainer.diffusion.main_flowgrpo \
    ...
    actor_rollout_ref.rollout.algo.sde_type="dance" \
    ...
    trainer.experiment_name=dancegrpo-diffusion-e2e \
...
echo "DanceGRPO diffusion e2e test passed (training completed successfully)."
