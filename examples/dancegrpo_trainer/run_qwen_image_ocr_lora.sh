# Qwen-Image lora RL via DanceGRPO, vllm_omni rollout
#
# DanceGRPO (https://arxiv.org/abs/2505.07818) shares the FlowGRPO scaffolding;
# the only on-policy difference is the SDE step formula (``sde_type=dance``)
# and the fact that every denoising step contributes a log-probability
# (``sde_window_size=null`` → spans the full trajectory).
set -x
